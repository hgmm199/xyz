...
function startClient(profileName, envPath) {
  ...
  client.once('ready', () => {
    ...
    // ✅ State độc lập mỗi profile — không share nhau
    const state  = {};
...
function startRootClient() {
  ...
  if (!token) {
    console.log(c.red('❌ No HTOKEN in .env — root client skipped.'));
    return;
  }
  if (!channelId) {
    console.log(c.red('❌ No CHANNEL in .env — root client skipped.'));
    return;
  }
